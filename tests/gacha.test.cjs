...
const state={coins:0,reviewResults:{},reviewIndex:0,round:PREFECTURES.slice(0,3),unlocked:new Set(),cleared:new Set(),current:PREFECTURES[0],screen:'review',gachaRewards:[],gachaFromReview:false,gachaBusy:false};
...
for(let i=0;i<3;i++){
  ...
  assert.equal(state.coins,1);assert.equal(state.screen,'review-coin');assert.equal(state.gachaFromReview,true);
  state.screen='gacha';state.gachaBusy=false;ctx.Math.random=()=>i/47;ctx.pullGacha();
  assert.equal(state.coins,0);assert.equal(state.screen,'reward-reveal');assert.equal(state.gachaFromReview,false);
}
...
console.log('PASS: each review immediately awards exactly one gacha pull, preserves three reveals, and records learning clears');
