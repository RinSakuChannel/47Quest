...
for(const scene of Object.keys(audio.scenes))for(let step=0;step<64;step++)audio.music(context,bus,scene,step,2+step*.25);
assert.equal(connected,voices,'every sound must go through its supplied volume bus');
console.log(`PASS: 47 distinct material recipes, ${Object.keys(audio.scenes).length} scene arrangements, ${voices} scheduled voices routed through volume control. Listening test not performed.`);
