...
for(const def of Object.values(definitions)){
  ...
  for(let i=0;i<890;i++)assert.equal(r.tick(.05),false,'must not end before 45 seconds');
  for(let i=0;i<20;i++)r.tick(.05);
  assert.equal(r.tick(.05),true);
  const practice=round(def.goal,def.time);
  for(let i=0;i<1000;i++)assert.equal(practice.tick(.05),false,'no failure at timeout');
  for(let i=0;i<def.goal;i++)practice.hit();
  assert.equal(practice.tick(.05),true,'practice completes after reaching target');
  ...
  const phase=round(def.goal);phase.elapsed=16;assert.equal(phase.phase,1);phase.elapsed=31;assert.equal(phase.phase,2);
...
for(const [code,def] of Object.entries(definitions)){
  ...
  assert.equal(sandbox.window.QUEST_MICROGAMES.catalog[code].time,45);
}
console.log('PASS: round duration, extended practice, stars, phases, scoring, catalog integration');
