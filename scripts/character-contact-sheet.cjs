...
(async()=>{
 ...
 for(let n=1;n<=47;n++){
  ...
  layers.push({input:await sharp(`assets/characters/v3/${code}.webp`).resize(170,175,{fit:'contain',background:'#e6e9e7'}).png().toBuffer(),left:x+5,top:y});
 ...
 await sharp({create:{width:1440,height:1230,channels:3,background:'#e6e9e7'}}).composite(layers).png().toFile('.verification/characters-audit.png');
})();
