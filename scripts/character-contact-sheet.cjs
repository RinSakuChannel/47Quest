...
(async()=>{
 ...
 vm.runInNewContext(fs.readFileSync('src/prefectures.js','utf8'),sandbox);
 ...
 for(let n=1;n<=47;n++){
  ...
  const svg=decodeURIComponent(sandbox.window.CHARACTER_ART[code].split(',')[1]);
  layers.push({input:await sharp(Buffer.from(svg)).resize(170,175,{fit:'contain',background:'#f8f5de'}).png().toBuffer(),left:x+5,top:y});
